...
let stdin = Fd.int64_of_file_descr (Unix.dup Unix.stdin) in
let stdout = Fd.int64_of_file_descr (Unix.dup Unix.stdout) in
let stderr = Fd.int64_of_file_descr (Unix.dup Unix.stderr) in
let () = Wamr.Functions.wasm_runtime_set_wasi_args_ex wasm_mod (Ctypes.CArray.start dirs) (Unsigned.UInt32.of_int 2) (nothing ()) (zero ()) (nothing ()) (zero ()) (nothing ()) 0 stdin stdout stderr in
