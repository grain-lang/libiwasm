...
let dir1 = Cstruct_cap.of_string "." in
let dir1_start = Ctypes.(bigarray_start array1 (Cstruct_cap.unsafe_to_bigarray dir1)) in
let dir2 = Cstruct_cap.of_string "/tmp" in
let dir2_start = Ctypes.(bigarray_start array1 (Cstruct_cap.unsafe_to_bigarray dir2)) in
let dirs = Ctypes.CArray.of_list Ctypes.(ptr char) [dir1_start; dir2_start] in
(* let () = Wamr.Functions.wasm_runtime_set_wasi_args_ex wasm_mod (Ctypes.CArray.start dirs) (Unsigned.UInt32.of_int 2) (nothing ()) (zero ()) (nothing ()) (zero ()) (nothing ()) 0 0 1 2 in *)
